...
//
// Modified as needed by Bojan Endrovski Copyright (c) 2009
#endregion
    ...
    public struct Complex : IEquatable<Complex>, IComparable<Complex>
    {
        ...
        public Complex Sign
        {
            get
            {
                ...
                {
                    return new Complex(Math.Sqrt(0.5), Math.Sqrt(0.5));
                }
        ...
        Tangent()
        {
            ...
            return new Complex(
                Math.Sin(real) * cosr / denom,
                sinhi * Math.Cosh(imag) / denom
                );
        }
        ...
        Cotangent()
        {
            if(IsReal)
            {
                return new Complex(1.0 / Math.Tan(real), 0d);
            }
        ...
        HyperbolicTangent()
        {
            ...
            return new Complex(
                Math.Cosh(real) * sinhr / denom,
                cosi * Math.Sin(imag) / denom
                );
        }
        ...
        SquareRoot()
        {
            ...
            {
                return new Complex(
                    Math.Sqrt(0.5) * Math.Sqrt(mod + real),
                    Math.Sqrt(0.5) * Math.Sqrt(mod - real)
                    );
            }
        ...
        private sealed class ComplexParser
        {
            ...
            char LookAheadCharacter
            {
                get
                {
                    if(cursor >= source.Length)
                    {
                        throw new ArgumentException(null, "complex");
                    }
